...
export async function POST(request: NextRequest) {
  try {
    ...
    const parentIds = [...new Set((parentLinks || []).map((l: any) => l.parent_user_id))] as string[];
    ...
    for (const parentId of parentIds) {
      ...
      if (remainingLinks?.length) {
        const otherIds = remainingLinks.map((l: any) => l.student_id);
        const { data: otherStudents } = await supabase
          .from('students')
          .select('school_id')
          .in('id', otherIds);
        hasOtherKidsAtSchool = (otherStudents || []).some((s: any) => s.school_id === student.school_id);
      }
