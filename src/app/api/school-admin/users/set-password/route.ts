...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const belongsToAdminSchool = (targetRoles || []).some((r: any) => schoolIds.includes(r.school_id));
    ...
    if (!allowed) {
      ...
      allowed = (parentLinks || []).some((link: any) => {
    ...
    const includesSuperAdmin = (targetRoles || []).some((r: any) => r.role === 'super_admin');
