...
export async function GET(request: NextRequest) {
  try {
    ...
    const pickupPersonsByStudent: Record<string, any> = queueResult.pickup_persons_by_student || {};
    ...
    const pickupNotices: EnrichedPickupNotice[] = (pickupNoticesRaw || []).map((n: any) =>
      enrichNotice(n as Record<string, unknown>)
    );
    const pickupRequests: EnrichedPickupRequest[] = (pickupRequestsRaw || []).map((r: any) =>
